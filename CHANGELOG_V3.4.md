# V3.4.2.2 — MCP Transport Hardening

## Purpose
V3.4.2.2 is a corrective deployment build based on V3.3. It preserves the V3.3 data-integrity architecture while hardening the WebSocket streaming acceptance tool against MCP `-32603 Invalid response format` failures and argument/default substitution issues.
...
## Important
V3.4.2.2 does not grant execution authorization. A successful WebSocket acceptance result remains a data-integrity result only. `NO_EVENT` remains distinct from stale data, and gateway latency is never used as market-data age.
